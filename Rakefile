...
task :test_app do
  ENV['LIB_NAME'] = 'solidus_manufacturers'
  Rake::Task['common:test_app'].invoke
end

require 'solidus_manufacturers'
namespace :solidus_manufacturers do
  ...
  task :sync do
    ...
    Dir["#{locales_dir}/*.yml"].each do |filename|
      ...
      (comments, other) = SolidusManufacturers::Utils.read_file(filename, basename)
      ...
      SolidusManufacturers::Utils.write_file(filename, basename, comments, other, false)
    end
  end

  def translation_keys
    (dummy_comments, words) = SolidusManufacturers::Utils.read_file(File.dirname(__FILE__) + '/default/solidus_core.yml', 'en')
      words
  end
